...
const { Engine, World, Bodies } = Matter;  // Matter.js
...
class Cue extends Edge {
    ...
    update(_sunk){
        this.pos.x = (cueActive) ? mouseX : (100);
        this.pos.y = (cueActive) ? mouseY : (tableHeight/2);
        ...
        if (!_sunk){
            ...
            wmbi.x = (cueActive) ? balls[0].pos.x : (100);
            wmbi.y = (cueActive) ? balls[0].pos.y : (280);
            wtpi.x = (cueActive) ? mouseX : (100);
            wtpi.y = (cueActive) ? mouseY : (tableHeight/2);
...
class CueBall extends Ball {
    ...
    updatePosition(x, y) {
        // Check if the new position is inside the "D" area
        ...
        if (distance <= outerRadius && y >= centerY - outerRadius && y <= centerY + outerRadius && x < tableHeight/2 +50) {
            this.pos.x = x;
            this.pos.y = y;
        }
    }
...
function setup(){
    ...
    // initiallly place ball at starting position
    setupBalls(1);
...
function draw(){
    ...
    if(!gameStarted){
        text(`Press 1 for Starting Position
Press 2 for Random Positions
Press 3 for Random Positions - Red Balls only`, 232, 32)
    }
...
function keyPressed() {
    if (key === '1' && gameStarted === false) {
        setupBalls(1);
    }
    else if (key === '2' && gameStarted === false) {
        setupBalls(2);
    }
    else if (key === '3' && gameStarted === false) {
        setupBalls(3);
    }
}

// Handle touch or mouse press
balls[0] instanceof cueBall;
function touchStarted() {
    if (balls[0].checkMouseInsideBall()) {
        console.log("clicked inside ball");
        balls[0].isDragging = true;
    } else {
        console.log("clicked outside ball");
        cueActive = true;
    }
...
function mouseDragged() {
    if (balls[0].isDragging) {
        balls[0].updatePosition(mouseX, mouseY);
    } else {
        cueActive = true;
    }
    return false; // Prevent default behavior
...
// Handle touch or mouse release
function touchEnded() {
    balls[0].isDragging = false;
    cueActive = false;
    return false; // Prevent default behavior
}
...
function setupBalls(mode) {
    ...
    if (mode === 1) {
        ...
        // Random positions
        // balls[0].pos.set(random(width), random(height));
        
        for (let i = 0; i < 22; i++) {
            let x = getRndInteger(210, 1290);
            let y = getRndInteger(120, 630);
            balls[i].pos.set(x, y);
        }
        // width/2,height/2,topEdge.wid, leftEdge.hid
    } 
    else if (mode === 3) {
        // Random positions - red balls only
